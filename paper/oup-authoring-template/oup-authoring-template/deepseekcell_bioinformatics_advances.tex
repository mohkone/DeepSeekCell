...
\abstract{
\textbf{Motivation:} Large language models (LLMs) can interpret marker genes for single-cell RNA sequencing (scRNA-seq) annotation, but a single model response is difficult to audit and repeated reasoning over every cluster is inefficient. A key methodological question is whether deterministic biological evidence can identify which clusters deserve a second LLM pass.\\
\textbf{Results:} We present DeepSeekCell, an evidence-guided framework for marker-based cell type annotation. DeepSeekCell performs structured first-pass LLM annotation, maps labels to the Cell Ontology, scores deterministic evidence from marker profiles, ontology support, tissue consistency and mixed-cluster signals, adjusts confidence scores, and selectively invokes a second LLM pass for conflicted clusters. In a paired six-dataset benchmark, all DeepSeekCell ablation arms reused the same cached first-pass response, separating first-pass prompting, evidence scoring, confidence adjustment and refinement. Evidence-guided selective refinement increased mean macro-F1 from 0.733 to 0.794 and recovered 59.1\% of the corrections achieved by full refinement while refining only 6.4\% as many clusters. Correction efficiency was 81.3\%, compared with 56.3\% for Random-k, 43.8\% for Confidence-k and 8.8\% for full refinement; no selector introduced correct-to-wrong revisions. Multi-model pilots with local Ollama models showed that Evidence-k remained the strongest selector across Llama 3.2, Mistral and Gemma2. A ZilionisLung ontology-disabled ablation retained most, but not all, of the Evidence-k benefit, supporting a joint contribution from marker and ontology evidence.\\
...
\caption{DeepSeekCell workflow. Cluster marker genes, tissue and species metadata are submitted through the R API or Shiny interface. The framework applies marker processing and prompt engineering, sends a structured prompt to the selected DeepSeek or Ollama backend, parses the JSON response, maps the annotation to the Cell Ontology, scores deterministic biological evidence, flags conflicted clusters for selective refinement, and exports tabular or HTML reports with provenance fields.}
\label{fig:workflow}
\end{figure}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\textwidth]{ollama_multimodel_all_selector_efficiency.pdf}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\textwidth]{ollama_multimodel_all_plain_selfrefined_delta.pdf}
